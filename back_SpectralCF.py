...
class SpectralCF(object):
    def __init__(self, K, graph, n_users, n_items, emb_dim, lr, batch_size, decay, DIR):
        ...
        if os.path.exists(eigenPickleFile):
            self.lamda, self.U = self.loadEigenvector(eigenPickleFile)
        else:
            self.lamda, self.U = self.genEigenvector(eigenPickleFile)    

        self.lamda = np.diag(self.lamda)
        # print("the shape of constant")
        # print(self.L.shape, self.lamda.shape, self.U.shape)
...
#         self.lamda = tf.placeholder(tf.float32, shape=[self.n_users+self.n_items, self.n_users+self.n_items])
#         self.U = tf.placeholder(tf.float32, shape=[self.n_users+self.n_items, self.n_users+self.n_items])
...
#         self.neg_i_embeddings = tf.nn.embedding_lookup(self.i_embeddings, self.neg_items)
        ...
        self.all_ratings = tf.matmul(batch_u_embeddings, self.i_embeddings, transpose_a=False, transpose_b=True)
